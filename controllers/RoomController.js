...
exports.createRoom = async function (req, res) {
    ...
    let foundedRoom = await Room.findOne({
        admin: req.user._id,
        users: uniqueList
    }).populate("users", '_id name email avatar last_seen', {
        _id: {
    ...
    if (!foundedRoom) {
        ...
        await room.save(async function (err) {
            if (err) {
                ...
                let roomCreated = await Room.findOne(room._id).populate("users", '_id name email avatar last_seen', {
                    _id: {
...
exports.getRooms = async function (req, res) {
    try {
        ...
        let foundedRoom = await Room.find({"users": {"$in": req.user._id}}).populate("users", '_id name email avatar last_seen', {
            _id: {
                $ne: req.user._id //except the current user
            }
        }).populate("last_msg_id").populate({
...
exports.getRoom = async function (req, res) {
    try {
        ...
        let foundedRoom = await Room.findById(id).populate("users", '_id name email avatar last_seen', {
            _id: {
                $ne: req.user._id //except the current user
            }
        }).populate("last_msg_id").populate({
